...
FILE* newReport(AllPaths paths)
{
    ...
    fprintf(pfileTxt, "Funções utilizadas no arquivo .qry\n");
    return pfileTxt;
}

void endReport(FILE* txt){
    fprintf(txt, "\nNo final tudo sempre da certo.");
    fclose(txt);
}
...
{
    Coord coord = getFirstCoord(queue);
...
void readQry(FILE* pTxt, YxxTree tree, AllPaths paths, PathsChecked pathsChecked)
{
    char filename[150];
    ...
    while(!feof(pfileQry))
    {
        ...
        if(strcmp(function, "inp") == 0){
            ...
            pol(pSvg, tree, queue, i, d, e, colorBorder, colorFill);
        }
